...
class MovingAverageControllerSpec extends WordSpec with MockFactory with Matchers {
  ...
  private val testRandomNumberGenerator = new Random(1000)
  ...
  private val FakeRequestGetMovingAverage = FakeRequest(GET, "/movingaverage")
  ...
  "movingAverage" should {
    "return valid moving averages" when {
      "startDate and endDates are valid" in {
        (bitCoinPriceMovingAverageServiceMock.average _).expects(ValidStartDate, ValidEndDate, MovingAverageNumber)
          .returning(TestExpectedMovingAverageFuture) once()
    ...
    "return InternalServerError" when {
      "bitCoinPriceMovingAverageService throws an Exception" in {
        (bitCoinPriceMovingAverageServiceMock.average _).expects(ValidStartDate, ValidEndDate, MovingAverageNumber)
          .returning(Future(throw new Exception)) once()
